...
export function clearDecode(
  ...
  const sizeOfPayloadSizeField = messageDataView.getUint8(0) & FlagMask;

  if (sizeOfPayloadSizeField === 0) return;

  const payloadSizeDataView = new DataView(
    message.buffer,
    start,
    sizeOfPayloadSizeField
  );
  const payloadSize = payloadSizeDataView.getUint32(0, true);

  start += sizeOfPayloadSizeField;
  const payload = message.slice(start, start + payloadSize);

  const isSigned = (messageDataView.getUint8(0) & IsSignedMask) == IsSignedMask;
  if (isSigned) {
...
function addPayloadSizeField(msg: Uint8Array, payload: Uint8Array): Uint8Array {
  ...
  fieldDataView.setInt32(0, payload.length, true);
...
function validateDataIntegrity(
  ...
  return expectedSize <= 3 || !!value.find((i) => i !== 0);
}
...
function ecRecoverPubKey(
  ...
  const signatureDataView = new DataView(signature.buffer);
  const recovery = signatureDataView.getUint8(64);
